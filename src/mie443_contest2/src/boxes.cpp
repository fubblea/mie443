...
bool Boxes::load_coords() {
  ...
  if (fs.isOpened()) {
    ...
    std::string coords_xml[5] = {"coordinate1", "coordinate2", "coordinate3",
    ...
    if (coords.size() == 0) {
      std::cout << "XML ERROR: Coordinate data is improperly formatted - check "
                   "input.xml"
                << std::endl;
      return false;
    }
  ...
  if (fs.isOpened()) {
    ...
    for (; it != end; ++it) {
      imagepath = ros::package::getPath("mie443_contest2") +
                  std::string("/boxes_database/") + std::string(*it);
